...
const NavbarPage = () => {
  const {acctivebutton,setacctivebutton,save,plan} = useContext(AppContext);
  const hendelstyleeven = (type: "Workouts" | "My Plan")=>{
    setacctivebutton(type)
  ...
  const links = (
    <>
      <li>
        <Link
          href="/"
          className={`${acctivebutton === "Workouts" ? "text-lime-400 border rounded-2xl  border-b-lime-200" : "" }`}
          onClick={()=>hendelstyleeven("Workouts")}
        >
          Workouts
        </Link>
      </li>
      <li>
        <Link href="/MyPlan" className={`${acctivebutton === "My Plan" ? "text-lime-400 border rounded-2xl  border-b-lime-200" : "" }`}
        onClick={()=>hendelstyleeven("My Plan")}>My Plan</Link>
      </li>
    </>
  );
  return (
